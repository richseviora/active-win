...
const user32 = new ffi.Library('User32.dll', {
	...
	EnumWindows: ['void', ['pointer']]
});

function getAllWindowPointers() {
	const results = [];
	// Callback from the native lib back into js
	const enumWindowCallback = ffi.Callback(
		'bool',
		['pointer', 'int'],
		function (pointer) {
	...
	user32.EnumWindows(enumWindowCallback);
...
function getForegroundWindow() {
	...
	const activeWindowId = ref.address(pointer);

	return getWindowDetail(activeWindowHandle, activeWindowId);
}

function getWindowDetail(pointer, activeWindowId) {
	...
	const windowId = ref.address(pointer);

	// Get the window text length in "characters" to create the buffer
	const windowTextLength = user32.GetWindowTextLengthW(pointer);
	...
	user32.GetWindowTextW(pointer, windowTextBuffer, windowTextLength + 2);
	...
	user32.GetWindowThreadProcessId(pointer, processIdBuffer);
	...
	const getWindowRectResult = user32.GetWindowRect(pointer, bounds.ref());
...
function windows() {
	...
	if (!ref.isNull(activeWindowHandle)) {
		activeWindowId = ref.address(activeWindowHandle);
	}

	return getAllWindowPointers()
		.map((pointer) => {
			return getWindowDetail(pointer, activeWindowId);
		})
		.filter((node) => {
...
module.exports = () => Promise.resolve(getForegroundWindow());
module.exports.sync = getForegroundWindow;
module.exports.allWindowsAsync = () => Promise.resolve(windows());
module.exports.allWindowsSync = windows;
